...
在定域极限下，$F(a)=\delta^4(a)$（动量空间中的定域极限是截断参数$\Lambda\to \infty$），
非定域拉氏量回到定域拉氏量。
我们很容易证明定域和非定域拉氏量\eqref{eq.2.36}和\eqref{eq.2.34}在以下规范变换下不变
...
\begin{equation}\begin{aligned}
        ...
        %%%%%%%%%%%%%%%
        \\  &{} =
        -i\eofphi \int_0^1 dt \int d^4a \, F(a) a^\mu \times
        \\  &{} \phantom{\times} 
        \bar{p}(x-at) 
        \left[
            \frac{C_{B_\phi}}{f}
        ...
        \\  &{} + \frac{\eofphi\,\coupleofphi}{2f^2}
        \int_0^1 dt \int d^4a F(a) \int d^4b 
        \\  &{} \phantom{\times} 
        (a-b)^\mu\bar{p}(x-at-b(1-t))
    ...
    从 \eqref{eq.2.45} 式子可以看出规范链接流$\delta J^\mu_q (x)$正比于$a^\mu$，
    这跟定域守恒流的洛伦兹结构不一样。
    并在定域极限$a\to 0$下，额外电磁流$\delta J^\mu_q (x)$为零。
    
    额外守恒流$\delta J^\mu_q(x)$引入一些额外的费曼图，它们使得规范不变性得以保持

    \section{本章总结}

    这一张我们主要讨论了手征对称性和手征拉氏量，并阐述了非定域手征拉氏量的构造过程。
    为了消除圈图计算中出现的紫外发散，引进了非定域正规化方案。
    跟其他正规化不同的是，非定域正规化在拉氏量层次消除紫外发散。
    为了保持规范不变性，非定域拉氏量中引入规范链接算符，
    因此产生额外的守恒流。
    此外，非丁羽手征拉氏量包含正规化因子$F(a)$，它反映了粒子的非定域效应。
    原来的拉氏量非定域化后圈积分是超重整的。
    利用这些非定域手征拉氏量，下一章我们研究在两种正规化方案
    （非定域正规和Pauli-Villars正规化）中的八重态重子和十重态重子分裂函数。