...
ping
最基本的网络命令是 ping。这个 ping 命令发送一个特殊的网络数据包,叫做IMCP ECHO_REQUEST,到 一台指定的主机。
...
ttl=43 time=105 ms
...
按下组合键 Ctrl-c,中断这个命令之后,ping 打印出运行统计信息。一个正常工作的网络会报告 零个数据包丢失。
...
第一个,叫做 eth0,是 因特网接口,和第二个,叫做 lo,是内部回环网络接口,它是一个虚拟接口,系统用它来 `自言自语`。

当执行日常网络诊断时,要查看的重要信息是每个网络接口第四行开头出现的单词 `UP`,
说明这个网络接口已经生效,还要查看第二行中 inet addr 字段出现的有效 IP 地址。
